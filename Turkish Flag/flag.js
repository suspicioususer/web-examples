...
function flag() {
	...
	*/
    ...
    context2.rotate((Math.PI * 4 / 10)); //initial offset rotation so our star is straight
    for (var i = 5; i--;) { // make a point, 5 times
        context2.lineTo(0, starRadius); // draw line up
        context2.translate(0, starRadius); // move origin to current same location as pen
        context2.rotate((Math.PI * 2 / 10)); // rotate the drawing board
        context2.lineTo(0, -starRadius); // draw line down
        context2.translate(0, -starRadius); // again, move origin to pen...
        context2.rotate(-(Math.PI * 6 / 10)); // ...and rotate, ready for next arm
    }
    context2.lineTo(0, starRadius); // last line to connect things up
    context2.closePath();
	context2.fillStyle = "white";
    context2.fill();
	context2.strokeStyle = "white"; // stroke the path, you could also .fill()
    context2.stroke();
	*/
